...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			...
			IMP_LOG_DBG(TAG, "%s %s \n", "------step 3 ------------------------------>","IMP_Encoder_CreateGroup()");
			IMP_LOG_DBG(TAG, "%s %d \n","           for channel : ",chn[i].index);
			if (ret < 0) {
	...
	IMP_LOG_DBG(TAG, "%s %s \n", "------step 6 ------------------------------>","sample_framesource_streamon()");
	/* Step.5 Stream On */
	...
	/* Step.6 Get stream */
    if (byGetFd) {
    	IMP_LOG_DBG(TAG, "%s %s \n", "------step 7 ------------------------------>","sample_get_video_stream_byfd()");
    	...
    	IMP_LOG_DBG(TAG, "%s %s \n", "------step 7 ------------------------------>","sample_get_video_stream()");
	...
	/* Step.a Stream Off */
	IMP_LOG_DBG(TAG, "%s %s \n", "------step 8 ------------------------------>","sample_framesource_streamoff()");
	...
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			IMP_LOG_DBG(TAG, "%s %s \n", "------step 9 ------------------------------>","IMP_System_UnBind()");
			IMP_LOG_DBG(TAG,  "%s %d \n","           unbind  channel index : ",chn[i].index);
	...
	IMP_LOG_DBG(TAG, "%s %s \n", "------step 11 ------------------------------>","sample_framesource_exit()");
	if (ret < 0) {
